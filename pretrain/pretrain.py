# This file is borrowed from https://github.com/Lightning-AI/litgpt/blob/main/litgpt/pretrain.py
# with the following changes:
# - Update FSDP with Activation checkpointing
# - Update FSDP with hybrid shard
# - Launch fabric with the number of nodes present in the cluster
...
def setup(
    ...
    if devices > 1:
        strategy = FSDPStrategy(auto_wrap_policy={Block}, state_dict_type="sharded", sharding_strategy="FULL_SHARD", activation_checkpointing_policy={Block})
    else:
        strategy = "auto"
    fabric = L.Fabric(devices=gpus_per_node, num_nodes=num_nodes, strategy=strategy, precision=precision, loggers=[logger])
    fabric.launch()
    ...
    if fabric.global_rank == 0:
        fabric.print("Saving hyper parameters")
        with open(out_dir / "final" / "hyperparameters.yaml", 'w') as yaml_file:
...
def fit(
    ...
    fabric.print(f"devices: {devices}, train.batch_size(devices): {train.batch_size(devices)}")
    log_iter_interval = train.log_interval * train.gradient_accumulation_iters(devices)
    initial_iter = state["iter_num"]
    train_iterator = CycleIterator(train_dataloader)
    fabric.print(f"max_tokens_per_device: {max_tokens_per_device}, tokens_per_iter: {tokens_per_iter}, max_iters: {max_iters}, log_iter_interval: {log_iter_interval}")
...
def get_dataloaders(
    ...
    fabric.print(f"train_dataloader: {len(train_dataloader)}, val_dataloader: {len(val_dataloader)}")
